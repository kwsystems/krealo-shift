/**
 * Textos de las nueve alertas al encargado, en español e inglés.
 *
 * SOBREVIVIO AL BORRADO DE `supabase/` A PROPOSITO. Su consumidor —el enviador
 * `sendManagerAlerts`— todavia no esta portado a Cloud Functions, asi que ahora
 * mismo no lo usa nadie. Se conserva igualmente porque son 200 lineas de texto
 * traducido y revisado, y su prueba de paridad
 * (`src/features/notifications/__tests__/alert-messages.test.ts`) sigue vigilando
 * que ningun idioma se quede corto. Rehacer esto seria trabajo tirado; tenerlo aqui
 * es media tarea menos cuando se porte el enviador.
 */
/**
